...
Since agent and master credentials are identified by [certname][], agent and master processes running on the same server use the same credentials.
...
* `private_keys` _(directory)_ --- Contains any private keys present on this node. This should generally only include the node's own private key, although on the CA it also contains any private keys created by the `puppet cert generate` command. It never contains the private key for the CA certificate. Mode: 0750. Setting: [`privatekeydir`][privatekeydir].
