...
Not all operating systems can manage the same resources with Puppet; some resource types are OS-specific, and others have features that are only applicable to some OSes. See the [resource type reference][] for details.
...
* **Make a cron job that runs Puppet agent.** Requires more manual configuration, but a good choice if you want to reduce the number of persistent processes on your systems. (This was popular with older Puppet and Ruby versions, before some high-profile performance and memory issues were fixed.)
...
This behavior is good for building a cron job that does configuration runs. You should also do one of the following to keep the Puppet master from being overwhelmed by hundreds of agents checking in at once:

* Randomize the time of the cron job across your fleet.
* Use the [`splay`][splay] and [`splaylimit`][splaylimit] settings to let Puppet randomize agent requests.
